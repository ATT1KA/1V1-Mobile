...
struct VictoryRecapView: View {
    ...
    private func generateShareableImage() async {
        ...
        if #available(iOS 16.0, *) {
            ...
            let controller = UIHostingController(rootView: shareableRecapView)
    ...
    private func saveToPhotos() async {
        let image: UIImage
        if let existingImage = shareableImage {
            image = existingImage
        } else {
            guard let generatedImage = await generateShareableImageSync() else { return }

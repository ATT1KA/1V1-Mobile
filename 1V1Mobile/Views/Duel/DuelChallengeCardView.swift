...
struct DuelChallengeCardView: View {
    ...
    private func loadQRCode() async {
        do {
            ...
            qrCodeImage = try await qrCodeService.generateQRCode(for: challengeCard.shareUrl)
        } catch {
    ...
    private func shareChallenge() async {
        ...
        let activityViewController = UIActivityViewController(
            activityItems: [shareText, qrCodeImage].compactMap { $0 },
...
extension DuelChallengeCardView {
    private var challengeDetailsView: some View {
        VStack(spacing: 20) {
            // Game Info Card
            VStack(spacing: 16) {
                HStack(spacing: 16) {
                    Image(systemName: gameIcon(for: challengeCard.gameType))
                        ...
                        .background(
                            Circle()
                                .fill(gameColor(for: challengeCard.gameType).opacity(0.2))
                        )
            ...
            VStack(spacing: 12) {
                HStack(spacing: 8) {
                    Image(systemName: isExpiringSoon ? "clock.badge.exclamationmark" : "clock.fill")
                        .foregroundColor(isExpiringSoon ? .red : .orange)
                        .font(.title3)
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        Text(timeUntilExpiration)
                            .font(.title2)
                            .fontWeight(.bold)
                            .foregroundColor(isExpired ? .red : isExpiringSoon ? .orange : .green)
                        
                        Text(formatDate(challengeCard.expiresAt))
                            .font(.caption)
                            .foregroundColor(.white.opacity(0.7))
                    }
                ...
                .padding(16)
                .background(
                    Color.white.opacity(isExpired ? 0.05 : isExpiringSoon ? 0.1 : 0.08)
                )
                .cornerRadius(12)
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(
                            isExpired ? Color.red.opacity(0.5) : 
                            isExpiringSoon ? Color.orange.opacity(0.5) : 
        ...
        VStack(spacing: 16) {
            Button(action: {
                ...
                HStack(spacing: 12) {
                    ...
                    Image(systemName: showQRCode ? "chevron.up" : "chevron.down")
                        .foregroundColor(.white.opacity(0.7))
                        .rotationEffect(.degrees(showQRCode ? 180 : 0))
                }
                .padding(16)
                .background(Color.white.opacity(0.1))
                .cornerRadius(12)
                    ...
                    if let qrImage = qrCodeImage {
                        VStack(spacing: 12) {
                            ...
                            Text(challengeCard.shareUrl)
                                ...
                                .cornerRadius(8)
                                .onTapGesture {
                                    UIPasteboard.general.string = challengeCard.shareUrl
                                }
                        }
                    } else {
                        RoundedRectangle(cornerRadius: 12)
